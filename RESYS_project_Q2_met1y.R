...
gsms <- "XXX11XXX1X1X1XXX1X1XX00001XXX0XX0X00010X00XXXX0000000"
sml <- c()
...
exEntrez <- subset(exEntrez, exEntrez[,29] %in% rowAnnotation[,1]) # Subset exEntrez with nuID in both exEntrez & nuIDtoEntrez
rownames(exEntrez) <- exEntrez[,29]
exEntrez <- exEntrez[,-29]
...
sml <- sml[order(sml)]
fl <- as.factor(sml)
labels <- c("no+metastases","metastases1y")
